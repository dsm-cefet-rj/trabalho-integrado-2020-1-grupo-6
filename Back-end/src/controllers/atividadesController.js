...
module.exports = {
    ...
    showfiltro: async (req, res, next) => {
        try {
           ...
           const { idUsuario: userId, status: estado, idDisciplina: disciplinaID } = req.query;

       
            if (estado != undefined) {
                const user = await Usuarios.findOne({ usuario: userId });
                const atividade = await Atividades.find({ idUsuario: user.id, status: estado, idDisciplina: disciplinaID });
                return res.json(atividade);
            }
            else {
                const user = await Usuarios.findOne({ usuario: userId });
                const atividade = await Atividades.find({ idUsuario: user.id, idDisciplina: disciplinaID });                        
                return res.json(atividade);
            }
        } 
        catch (err) {
    ...
    showfiltroAtividades: async (req, res, next) => {
        try {
           ...
           const { idUsuario: userId, status: estado } = req.query;
            ...
            if (estado != undefined) {
                const user = await Usuarios.findOne({ usuario: userId });
                const atividade = await Atividades.find({ idUsuario: user.id, status: estado});
                return res.json(atividade);
            }
            else {
                const user = await Usuarios.findOne({ usuario: userId });
